...
const ExportCollectionDialog = (props) => { // Notice the arrow function... regular function()  works too
    ...
    // Variables ===
        ...
        const { getParentData, setParentAlert, selectedDataItemsList,
            ...
            exportCollectionDialogExecuting, setExportCollectionDialogExecuting  } = props;

        const [pdfWidth, setPdfWidth] = useState(defaultPdfWidth);
        const [pdfHeight, setPdfHeight] = useState(defaultPdfHeight);

        const [totalUserChapters, setTotalUserChapters] = useState(0);
    ...
    // Functions ===
        ...
        const exportCollectionChapters = useCallback(() =>
        {
            if(pdfWidth > 0 && pdfHeight > 0 && selectedDataItemsList.length > 0)
            {
                ...
                {
                    item.memberChapters.forEach(subItem => {
                ...
                Promise.all(promises).then(() => {
                    setParentAlert(new AlertType(`Successfully exported ${totalUserChapters} chapter${totalUserChapters > 1? "s" : ""}.`, "success"));
                })
                .catch(() =>{
                    setParentAlert(new AlertType(`Unable export ${totalUserChapters} chapter${totalUserChapters > 1? "s" : ""}. Please refresh and try again.`, "error"));
                });
            }
            else
            {
        ...
        }, [ pdfWidth, pdfHeight, selectedDataItemsList, totalUserChapters ]);
    ...
    // Hooks ===
        ...
        useEffect( () =>
        {
            ...
            {
                setTotalUserChapters(() => {
                    let sum = 0;
                    selectedDataItemsList.forEach(item => {
                        sum+=item.memberChapters.length
                    });
                    return sum;
                });
            }
                    ...
                    >
                        ...
                        <DialogContent>
                            {exportCollectionDialogExecuting? (
                                ...
                                <>
                                    <DialogContentText>
                                        Exporting {selectedDataItemsList.length} collection{selectedDataItemsList.length > 1? "s" : ""}, totalling {totalUserChapters} chapter{totalUserChapters > 1? "s" : ""}. Please enter valid pdf width and height values for your export.
                                    </DialogContentText>
